...
<body onload="startCountdown()">
    <div class="login-container">
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            $sql = "INSERT INTO client (clientName, status, address, email, password, registrationDate, company, phoneNumber) values (?,?,?,?,?,?,?,?)";
            $res = $conn->prepare($sql);
            $res->bind_param("ssssssss", $name, $status, $address, $email, $password, $registrationDate, $company, $phoneNumber);
            if ($res->execute()) {
                echo "<p>New Client Added Successfully. Employee Dashboard in <span id='timer'>10</span> seconds.</p>";
            } else {
                echo "Error inserting client: " . $stmt->error . "<br>";
            }
            $res->close();
        }
