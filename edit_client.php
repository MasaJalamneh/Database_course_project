            ...
            <section>
                ...
                <div class="login-container">
                    ...
                    <form method="POST" action="">
                        ...
                        <label for="attribute_to_update">Choose Attribute to Update:</label>
                        <select name="attribute_to_update" id="attribute_to_update">
                            <option value="clientName">Client Name</option>
                            <option value="status">Client Status</option>
                            <option value="address">Client Address</option>
                            <option value="email">Client Email</option>
                            <option value="registrationDate">Client Registration Date</option>
                            <option value="company">Client Company</option>
                            <option value="phoneNumber">Client Phone Number</option>
                        </select><br>
                        <label for="new_value">New Value:</label><br>
                        <input type="text" name="new_value" id="new_value" value="" placeholder="Enter new value">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] === "POST") {
                        ...
                        $attribute_to_update = isset($_POST['attribute_to_update']) ? $_POST['attribute_to_update'] : null;
                        $new_value = isset($_POST['new_value']) ? trim($_POST['new_value']) : null;

                        if ($clientID && $attribute_to_update && $new_value) {
                            $sql = "UPDATE client SET $attribute_to_update = ? WHERE clientID = ?";
                            $res = $conn->prepare($sql);
                            $res->bind_param("si", $new_value, $clientID); 
                            if ($res->execute()) {
                                echo "<div style='display: flex; justify-content: center; align-items: center; height: 100vh; text-align: center; flex-direction: column; color: red; font-size: 30px'>Client updated successfully!</div><br>";
                            } else {
                                echo "Error updating client: " . $res->error . "<br>";
                            }
                            $res->close();
                        } else {
